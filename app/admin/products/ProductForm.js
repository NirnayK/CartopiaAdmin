...
const Form = (props) => {
    ...
    const handleBack = async () => {
        ...
        const filteredImages = deletedImages.filter((image) => !images.includes(image));
        try {
            if (filteredImages.length > 0) {
                await axios.post('/api/images', filteredImages)
            }
            router.back();
        }
        catch (error) {
    ...
    return (
        <>
            <form className='space-y-3' onSubmit={handleSubmit}>
                ...
                {method !== 'DELETE' && (
                    <CldUploadButton className="bg-slate-200 cursor-pointer text-gray-600 btn" uploadPreset="qhcyym20" readOnly={method == 'DELETE'} onUpload={(e) => uploadPhotos(e)} />
                )}

                <div className='flex flex-wrap gap-5 p-2'>

                    {images && images.map((image) => (
                        <div key={image} className='relative w-[200px] h-[200px] rounded-md overflow-hidden p-1'>
                            {method !== 'DELETE' &&
